...
// (void)accuracy_normal;

double accuracy_normal = 0;

  if ((*borders)[0] == 0 && (*borders)[1] == 0) {
  accuracy_normal = computePredictionAccuracyInternal();
  } else {
  accuracy_normal = computePredictionAccuracyInternalCustom();
  }
...
// Randomly permute for all independent variables
  for (size_t i = 0; i < num_independent_variables; ++i) {
	...
	// (void)accuracy_permuted;
	
	permuteAndPredictOobSamples(varID, permutations);
		
		double accuracy_permuted = 0;
		
      if ((*borders)[0] == 0 && (*borders)[1] == 0) {
  accuracy_permuted = computePredictionAccuracyInternal();
  } else {
  accuracy_permuted = computePredictionAccuracyInternalCustom();
  }
...
// Use fraction (default 63.21%) of the samples
  ...
  // Kommentar Roman: In folgender Funktion bleibt der Code haengen:
