...
#include "Data.h"
 ...
 TreeRegression::TreeRegression(std::vector<std::vector<size_t>>& child_nodeIDs, std::vector<size_t>& split_varIDs,
                                std::vector<double>& split_values, std::vector<bool>* is_ordered_variable, std::vector<std::vector<double>>& endnodeprobs) :
   Tree(child_nodeIDs, split_varIDs, split_values, is_ordered_variable), counter(0), sums(0), endnodeprobs(endnodeprobs) {
 }
 ...
 std::vector<double> TreeRegression::CalculateProbs(size_t nodeID) {
   ...
   for (size_t i = 0; i < num_samples_in_node; ++i) {
     ...
     // Get the value of the metric response:
     ...
     // Add this count to the table of class values:
     ++tempsums[count];
 ...
 void TreeRegression::createEmptyNodeInternal() {
   endnodeprobs.push_back(std::vector<double>());
 }
 ...
 double TreeRegression::computePredictionAccuracyInternalCustom() {
   ...
   for (size_t i = 0; i < num_predictions; ++i) {
     size_t terminal_nodeID = prediction_terminal_nodeIDs[i];
     double predicted_value = split_values[terminal_nodeID];
     bool stillsmaller = true;
     size_t repsi = 0;
     if (predicted_value > (*borders)[0]) {
       while (stillsmaller && (*borders).size() > repsi + 1) {
         if (predicted_value <= (*borders)[repsi+1]) {
           stillsmaller = false;
         }
         repsi++;
       }
       if((*borders).size() == repsi + 1) {
         repsi++;
       }
       repsi++;
     } else {
     ...
     size_t predicted_class = repsi;
     
     double real_value = data->get(oob_sampleIDs[i], dependent_varID);
     
     stillsmaller = true;
     repsi = 0;
     if (real_value > (*borders)[0]) {
       while (stillsmaller && (*borders).size() > repsi + 1) {
         if (real_value <= (*borders)[repsi+1]) {
           stillsmaller = false;
         }
         repsi++;
       }
   ...
   for (size_t i = 0; i < num_predictions; ++i) {
     ...
     size_t terminal_nodeID = prediction_terminal_nodeIDs[i];
     std::vector<double> predicted_probs = endnodeprobs[terminal_nodeID];//
	 ...
	 for (size_t j = 0; j < nclass; ++j) {
		...
		cumsumtemp += predicted_probs[j];
		
		tempsum += (cumsumtemp - indtemp) * (cumsumtemp - indtemp);
		
	 }
	 
	 
   }
   return (- tempsum / (((double) num_predictions) * ((double) nclass - 1)));
