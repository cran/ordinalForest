...
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint mtry,
  ...
  *verbose_out << "Loading input file: " << input_file << "." << std::endl;
  ...
  // Kommentar Roman
  if (!case_weights_file.empty()) {
    std::vector<std::vector<double>> case_weights;
    case_weights.resize(1);
    loadDoubleVectorFromFile(case_weights[0], case_weights_file);
    if (case_weights[0].size() != num_samples - 1) {
      throw std::runtime_error("Number of case weights is not equal to number of samples.");
    }
    setCaseWeightVector(case_weights);
  }
...
void Forest::init(std::string dependent_variable_name, MemoryMode memory_mode, Data* input_data, uint mtry,
  ...
  // Kommentar Roman
  // Init case weights
  case_weights.push_back(std::vector<double>());
...
void Forest::grow() {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
	...
	// Kommentar Roman
	// Get case weights for tree
    std::vector<double>* tree_case_weights;
    if (case_weights.size() > 1) {
      tree_case_weights = &case_weights[i];
    } else {
      tree_case_weights = &case_weights[0];
    }
...
// Kommentar Roman
void Forest::setCaseWeightVector(std::vector<std::vector<double>>& case_weights) {

// Size should be 1 x num_samples or num_trees x num_samples
  if (case_weights.size() != 1 && case_weights.size() != num_trees) {
    throw std::runtime_error("Size of case weights not equal to 1 or number of trees.");
  }

// Reserve space
  if (case_weights.size() == 1) {
    this->case_weights[0].resize(num_samples);
  } else {
    this->case_weights.clear();
    this->case_weights.resize(num_trees, std::vector<double>(num_samples));
  }
  
  // Set weights
  for (size_t i = 0; i < case_weights.size(); ++i) {
    
    // Size should be 1 x num_independent_variables or num_trees x num_independent_variables
    if (case_weights[i].size() != num_samples) {
      throw std::runtime_error("Number of case weights is not equal to number of samples.");
    }
    
    for (size_t j = 0; j < case_weights[i].size(); ++j) {
      double weight = case_weights[i][j];

       if (weight <= 1 && weight >= 0) {
          this->case_weights[i][j] = weight;
        } else if (weight < 0 || weight > 1) {
          throw std::runtime_error("One or more case weights not in range [0,1].");
        }
