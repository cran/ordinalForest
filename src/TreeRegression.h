...
#include "Tree.h"
 ...
 public:
   ...
   std::vector<double> getEndNodePrediction(size_t sampleID) const {
     size_t terminal_nodeID = prediction_terminal_nodeIDs[sampleID];
     return (endnodeprobs[terminal_nodeID]);
   }
   ...
   size_t getPredictionTerminalNodeID(size_t sampleID) const {
    ...
    const std::vector<std::vector<double>>& getSplitValuesAll() const {
    return endnodeprobs;
  }
 ...
 private:
   ...
   double computePredictionAccuracyInternalCustom();
   ...
   double* sums;
     ...
     // For terminal nodes the response values of all target values are stored here:
  std::vector<std::vector<double>> endnodeprobs;
