simulateborders <-
function(J, ndiv=300, npermtrial=500) {
  ...
  if(nperm < ndiv) {
    ...
    # Repeat individual permutation tupels until they have the number 'ndiv':
    repind <- rep(1:nperm, each=floor(ndiv/nperm))
    repind <- c(repind, sample(1:nperm, size=ndiv - length(repind)))
    permind <- permmat[repind,]
  } 
  else {
    # Initialize matrix of rankings:
    permind <- matrix(nrow=ndiv, ncol=J)
  ...
  bordersbraw <- t(rbind(0, replicate(ndiv, sort(runif(J-1))), 1))
